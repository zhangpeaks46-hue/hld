<?php
/**
 * Deepseek（深度求索）AI 客户端
 * API 文档：https://api-docs.deepseek.com/
 * 接口风格与 OpenAI Chat Completions 兼容
 */
class DeepseekClient implements AIClient
{
    ...
    {
        return $this->cfg['model'] ?? 'deepseek-chat';
    }
    ...
    public function processDocument($documentText, $formatRequirements, $serviceType)
    {
        ...
        $resp = $this->httpPost(
            rtrim($this->cfg['base_url'], '/') . '/v1/chat/completions',
            $payload
        );
    ...
    private function httpPost($url, $payload)
    {
        if (empty($this->cfg['api_key'])) {
            throw new RuntimeException('Deepseek API Key 未配置，请在 api/config/ai_config.php 中填写');
        }
        ...
        if ($err) {
            throw new RuntimeException('Deepseek 请求失败: ' . $err);
        }
        if ($code >= 400) {
            throw new RuntimeException('Deepseek 返回错误 ' . $code . ': ' . $body);
        }
